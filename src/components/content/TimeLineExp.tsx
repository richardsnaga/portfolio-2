...
const TimelineEventTitle = ({ children }: { children: React.ReactNode }) => (
  <h1 className="text-xl text-black dark:text-white">{children}</h1> // Dark theme: white text
);

// TimelineEvent.Subtitle for event subtitle
...
// TimelineEvent.List for event list
const TimelineEventList = ({ children }: { children: React.ReactNode[] }) => {
  return (
    <ul className="list-disc pl-5 space-y-1">
      {children.map((child, index) => (
        <li key={index} className="text-base text-black dark:text-white">
          {child}
        </li>
      ))}
    </ul>
...
TimelineEvent.List = TimelineEventList;
